...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/select-character`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      res.status(response.status).json({ message: 'Character selection failed' });
    }
  } catch (error) {
